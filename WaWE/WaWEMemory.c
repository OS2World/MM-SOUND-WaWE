 ...
 * v1.1 - Added support for DosAllocMem() with OBJ_ANY flag                  *
 *****************************************************************************/
...
static void *huge_malloc_core(size_t size)
{
  APIRET rc;
  void *pResult;
...
static void huge_free_core(void *pTemp)
{
  DosFreeMem(pTemp);
}

static void *huge_realloc_core(void *old_blk, size_t old_size, size_t new_size)
{
  void *new_blk;

  if (!old_blk)
    return huge_malloc_core(new_size);

  new_blk = huge_malloc_core(new_size);
  if (new_blk)
  {
    if (old_size<new_size)
      memcpy(new_blk, old_blk, old_size);
    else
      memcpy(new_blk, old_blk, new_size);

    huge_free_core(old_blk);
  }
...
#ifdef OVERWRITE_DETECTION_AREA
      if (bIsHuge)
        pTemp->pPtr = huge_malloc_core(numbytes + 2*OVERWRITE_DETECTION_AREA);
      else
        pTemp->pPtr = malloc(numbytes + 2*OVERWRITE_DETECTION_AREA);

#else
      if (bIsHuge)
        pTemp->pPtr = huge_malloc_core(numbytes);
      else
...
void *dbg_realloc_core( void *old_blk, size_t size, char *pchSrcFile, int iLineNum )
{
  ...
  {
    // By definition
    return dbg_malloc_core(size, "Unknown", 0, 0);
  }
  ...
  if (DosRequestMutexSem(MemoryUsageDebug_Sem, SEM_INDEFINITE_WAIT)==NO_ERROR)
  {
    ...
    if (pTemp->bIsHugePtr)
      newptr = huge_realloc_core(pTemp->pRealPtr, pTemp->uiSize, size+2*OVERWRITE_DETECTION_AREA);
    else
    ...
    if ((pTemp) && (pTemp->bIsHugePtr))
      newptr = huge_realloc_core(old_blk, pTemp->uiSize, size);
    else
...
#ifdef OVERWRITE_DETECTION_AREA
      if (pTemp->bIsHugePtr)
        huge_free_core(pOld->pRealPtr);
      else
        free(pOld->pRealPtr);
#else
      if (pTemp->bIsHugePtr)
        huge_free_core(pOld->pPtr);
      else
        free(pOld->pPtr);
#endif
      free(pOld);
